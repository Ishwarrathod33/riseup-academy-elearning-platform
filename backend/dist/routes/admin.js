...
adminRouter.put("/lectures/:lectureId", ...adminOnly, asyncRoute(async (req, res) => {
...
adminRouter.delete("/lectures/:lectureId", ...adminOnly, asyncRoute(async (req, res) => {
...
adminRouter.get("/students", ...adminOnly, asyncRoute(async (req, res) => {
    ...
    const latestQuizPct = new Map();
    for (const a of quizAttempts) {
        const key = `${a.userId}:${a.quizId}`;
        if (latestQuizPct.has(key))
            continue;
    ...
    const lectureSumByUser = new Map();
    for (const uid of userIds)
        lectureSumByUser.set(uid, { sum: 0 });
    ...
    const quizSumByUser = new Map();
    for (const uid of userIds)
        quizSumByUser.set(uid, { sum: 0 });
    for (const uid of userIds) {
        if (!quizIds.length)
            continue;
        let sum = 0;
    ...
    for (const uid of userIds)
        attendanceCounts.set(uid, { PRESENT: 0, ABSENT: 0, LEAVE: 0 });
