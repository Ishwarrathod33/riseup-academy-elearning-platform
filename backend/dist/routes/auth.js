...
authRouter.post("/register", asyncRoute(async (req, res) => {
    try {
        ...
        if (e instanceof Prisma.PrismaClientKnownRequestError) {
            if (e.code === "P2002") {
                const fields = e.meta?.target?.join(", ") ?? "email or phone";
...
authRouter.post("/otp/verify", asyncRoute(async (req, res) => {
    ...
    if (challenge.attempts >= challenge.maxAttempts)
        return res.status(429).json({ error: "OTP attempts exceeded" });
    const otpHash = sha256Hex(otp);
    ...
    const userWhere = channel === "email" ? { email: normalized } : { phone: normalized };
    const user = await prisma.user.findUnique({ where: userWhere }).catch(() => null);
    if (!user)
        return res.status(500).json({ error: "User not found after OTP verification" });
    if (!user.isActive)
        return res.status(403).json({ error: "Account disabled" });
    const accessToken = await issueSession(user, res);
    const profileIncomplete = !(user.name && user.name.trim().length > 0);
...
authRouter.post("/refresh", asyncRoute(async (req, res) => {
    ...
    if (stored.tokenHash !== sha256Hex(refresh))
