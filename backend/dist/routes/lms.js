...
lmsRouter.get("/lectures", requireAuth, asyncRoute(async (req, res) => {
    ...
    const enrolled = await verifyEnrollment(req.user.userId, courseId);
    if (!enrolled) {
...
lmsRouter.post("/progress/lecture/:lectureId", requireAuth, asyncRoute(async (req, res) => {
    ...
    const enrolled = await verifyEnrollment(userId, lecture.courseId);
    if (!enrolled) {
        return res.status(403).json({
            error: "Course access denied",
        });
    ...
    const progressPct = durationSec && durationSec > 0
...
lmsRouter.get("/quizzes/:quizId", requireAuth, asyncRoute(async (req, res) => {
    ...
    const enrolled = await verifyEnrollment(req.user.userId, quiz.courseId);
    if (!enrolled) {
        return res.status(403).json({
            error: "Course access denied",
        });
...
lmsRouter.post("/quizzes/:quizId/submit", requireAuth, asyncRoute(async (req, res) => {
    ...
    if (!quizWithQuestions)
        return res.status(404).json({ error: "Quiz not found" });
    const enrolled = await verifyEnrollment(userId, quizWithQuestions.courseId);
    if (!enrolled) {
        return res.status(403).json({
            error: "Course access denied",
        });
...
lmsRouter.post("/certificates/generate/:courseId", requireAuth, asyncRoute(async (req, res) => {
    const userId = req.user.userId;
...
lmsRouter.get("/certificates/:certificateId/download", requireAuth, asyncRoute(async (req, res) => {
    const userId = req.user.userId;
    ...
    doc
        .fillColor("#000000")
        .fontSize(11)
        .text(`Certificate ID: RUA-${certificate.id.slice(0, 8)}`, 620, 45);
    // Title
    doc
        .fillColor("#D4AF37")
        .fontSize(46)
    ...
    if (fs.existsSync(signaturePath)) {
        doc.image(signaturePath, 90, 455, {
            width: 120,
        });
    }
    doc
    ...
    doc
        .fontSize(12)
        .fillColor("#000000")
        .text(`Issued: ${new Date(certificate.issuedAt).toLocaleDateString()}`, 620, 520);
    doc.end();
}));
