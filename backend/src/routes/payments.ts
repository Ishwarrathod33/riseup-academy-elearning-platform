...
function computeRazorpaySignature(orderId: string, paymentId: string) {
...
paymentsRouter.post("/verify", requireAuth, asyncRoute(async (req, res) => {
  ...
  if (payment.userId !== req.user!.userId) return res.status(403).json({ error: "Forbidden" });

  if (payment.status === "PAID") return res.json({ ok: true, enrolled: true });
